...
add_filter( "acll_cleaner_scripts", "prefix_stop_script_foo", 10, 1 );
...
add_filter( "acll_cleaner_styles", "prefix_stop_style_bar", 10, 1 );
```
...
add_filter( "acll_loader_script_handles_header", "prefix_send_script_foo_to_frontend", 10, 1 );
// Send info about script `foo` to the frontend. If `foo` is meant to be enqueued in footer.
add_filter( "acll_loader_script_handles_footer", "prefix_send_script_foo_to_frontend", 10, 1 );
...
add_filter( "acll_loader_style_handles", "prefix_send_style_bar_to_frontend", 10, 1 );
...
add_filter( 'wp_enqueue_scripts', 'prefix_enqueue_loader_script' );
```
